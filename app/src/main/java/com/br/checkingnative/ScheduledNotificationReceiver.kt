...
class ScheduledNotificationReceiver : BroadcastReceiver() {
    ...
    private fun ensureChannel(context: Context) {
        ...
        val channel = NotificationChannel(
            ...
            description = "Notificacoes agendadas de Check-In e Check-Out"
    ...
    companion object {
        private const val PREFS_NAME: String = "checking_schedule_prefs"
        private const val KEY_SCHEDULE_IN_ENABLED: String = "schedule_in_enabled"
        private const val KEY_SCHEDULE_IN_TIME: String = "schedule_in_time"
        private const val KEY_SCHEDULE_OUT_ENABLED: String = "schedule_out_enabled"
        private const val KEY_SCHEDULE_OUT_TIME: String = "schedule_out_time"
        private const val KEY_SCHEDULE_DAYS: String = "schedule_days"

        const val EXTRA_TIPO: String = "tipo"
        const val EXTRA_NOTIFICATION_ID: String = "notif_id"
        const val EXTRA_HORA: String = "hora"
        const val EXTRA_MINUTO: String = "minuto"
        const val EXTRA_DIAS_JSON: String = "dias_json"

        const val NOTIFICATION_ID_IN: Int = 20001
        const val NOTIFICATION_ID_OUT: Int = 20002

        private const val CHANNEL_ID: String = "checking-schedule-channel"
        private const val CHANNEL_NAME: String = "Agendamentos de ponto"
        ...
        fun saveScheduleConfig(
            ...
            context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit {
                ...
                putString(KEY_SCHEDULE_DAYS, JSONArray(scheduleDays).toString())
            }
        }
        ...
        fun calcularProximoHorario(hora: Int, minuto: Int, diasPermitidos: List<Int>): Long {
            ...
            if (diasPermitidos.isNotEmpty()) {
                for (unusedIndex in 0 until 7) {
                    val jsDayOfWeek = target.get(Calendar.DAY_OF_WEEK) - 1
                    if (diasPermitidos.contains(jsDayOfWeek)) {
                        break
                    }
                    target.add(Calendar.DAY_OF_MONTH, 1)
                }
        ...
        fun scheduleAlarm(
            ...
            val intent = Intent(context, ScheduledNotificationReceiver::class.java).apply {
                ...
                putExtra(EXTRA_DIAS_JSON, JSONArray(diasPermitidos).toString())
            }
        ...
        private fun parseDiasJson(diasJson: String): List<Int> {
            return try {
                val array = JSONArray(diasJson)
                (0 until array.length()).map { index -> array.getInt(index) }
            } catch (_: Exception) {
                emptyList()
            }
        }
